...
interface UseInvoiceItemsManagerProps {
  ...
  selectedContractSpec: any[];
  invoiceProductOptions: any[];
}

export function useInvoiceItemsManager({
  items,
  setItems,
  selectedContractSpec,
  invoiceProductOptions
}: UseInvoiceItemsManagerProps) {
  ...
  const [expandedSublists, setExpandedSublists] = useState<Set<number>>(new Set());

  const toggleItemSublist = (index: number) => {
    const newSet = new Set(expandedSublists);
    if (newSet.has(index)) newSet.delete(index);
    else newSet.add(index);
    setExpandedSublists(newSet);
  };
